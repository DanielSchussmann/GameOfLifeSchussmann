...
class GameOfLife():
    ...
    def _next_grid(self, grid):
        ...
        for row, column in np.ndindex(current_grid.shape):
            num_alive = np.sum(current_grid[row - 1:row + 2, column - 1:column + 2]) - current_grid[row, column]

            if current_grid[row, column] == 1 and num_alive < 2 or num_alive > 3:
               current_grid[row, column] = 0

            elif (current_grid[row, column] == 0 and  num_alive == 3):
